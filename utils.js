...
module.exports.findDeviceByIp = async (arr, targetIp) => {
  const sortedarrayOfObject = arr.sort((a, b) => a.ip.localeCompare(b.ip));

  let left = 0;
  let right = sortedarrayOfObject.length - 1;

  while (left <= right) {
    const mid = Math.floor((left + right) / 2);
    if (sortedarrayOfObject[mid].ip === targetIp) {
      return sortedarrayOfObject[mid];
    } else if (sortedarrayOfObject[mid].ip < targetIp) {
      left = mid + 1;
    } else {
      right = mid - 1;
    }
